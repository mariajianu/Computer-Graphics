...
void Laborator8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	float rad = glGetUniformLocation(shader->program, "rad");
	glUniform1f(rad, rad);
...
void Laborator8::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_H)) lightPosition = glm::rotate(glm::mat4(1), deltaTime * speed, glm::vec3(1,0,0))*glm::vec4(lightDirection,1);
		if (window->KeyHold(GLFW_KEY_J)) lightPosition = glm::rotate(glm::mat4(1), -deltaTime * speed, glm::vec3(1, 0, 0))*glm::vec4(lightDirection, 1);
		if (window->KeyHold(GLFW_KEY_K)) lightPosition = glm::rotate(glm::mat4(1), deltaTime * speed, glm::vec3(0, 1, 0))*glm::vec4(lightDirection, 1);
		if (window->KeyHold(GLFW_KEY_L)) lightPosition = glm::rotate(glm::mat4(1), -deltaTime * speed, glm::vec3(0, 1, 0))*glm::vec4(lightDirection, 1);
